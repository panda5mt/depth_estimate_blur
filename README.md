...
- [長船blur](./f_blur.m) : ガウシアンフィルタの代替
    ...
    対する長船blurはこれを[x軸方向に1次元でガウスフィルタ](./f_blur.m#L15)のたたみ込み，[y軸方向に1次元でメディアンフィルタを実行](./f_blur.m#L18)し，[元画像からの差分を計算](./main.m#L39)し，Sparseな深度を計算する．[長船によりFPGA向けに設計されたアルゴリズムであり](https://github.com/osafune/)，y軸方向はメディアンを採用したことにより，画像1枚分全部をRAMに保持する必要がなくなる(メディアン長さ分のみ保持すればよい)．組み込み向けデバイスのような省メモリデバイスでも動作し，高速に動作する．
...
- [大津の二値化](./my_graythresh.m) : superpixelの代替
    ...
    今回は，大津の<u>3値化</u>を行い，簡易セグメンテーションを行っている．
